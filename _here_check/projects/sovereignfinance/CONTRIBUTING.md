# Contributing to SovereignFinance

Thank you for considering a contribution! Please read this guide to help us maintain a high-quality codebase.
...
2. **Clone your fork** locally:
   ```bash
   git clone https://github.com/your-username/sovereignfinance.git
   cd sovereignfinance
   ```
...
- **Constants**: Place in `src/sovfin/config.py` and annotate with `Final`.
- **Avoid**: Magic numbers, deep nesting (>4 levels), long functions (>50 lines).

## Testing

- Write unit tests for new functionality in the appropriate `tests/` file.
- Aim for high coverage; at minimum, new code must be covered by tests.
- Use `pytest` fixtures for reusable setup (see `tests/conftest.py`).
...
Thank you again for contributing to SovereignFinance!