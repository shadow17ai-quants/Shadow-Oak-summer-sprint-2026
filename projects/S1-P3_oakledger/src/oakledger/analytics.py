...
def compute_metrics(df: pd.DataFrame) -> dict:
    ...
    empty = {
        ...
        "sharpe_trades": 0.0, "expectancy": 0.0, "rr_ratio": 0.0,
    }
...
def behavioral_metrics(df: pd.DataFrame) -> dict:
    ...
    empty = {"overconfidence_rate": 0.0, "recency_bias": 0.0, "loss_aversion_ratio": 0.0}
    if df.empty or 'result' not in df.columns:
        return empty
    df = df.copy()
    df['result'] = pd.to_numeric(df['result'], errors='coerce')
    ...
    return {"overconfidence_rate": over, "recency_bias": recency, "loss_aversion_ratio": loss_aversion}
...
def monthly_returns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    return d.groupby('month', as_index=False)['result'].sum().rename(columns={'result': 'pnl'})