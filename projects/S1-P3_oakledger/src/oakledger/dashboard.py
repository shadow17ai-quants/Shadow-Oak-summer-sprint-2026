...
from oakledger.analytics import (
    behavioral_metrics,
    compute_metrics,
    load_trades,
    monthly_returns,
...
if page == "Dashboard":
    ...
    with col_perf:
        ...
        total_gain = (
            m["max_profit"] * m["win_trades"]
            if m["win_trades"]
            else m["total_pnl"] * 0.6
        )
        wins_sum = df.copy()
    ...
    with col_wl:
        ...
        fig = go.Figure(
            data=[
                go.Pie(
                    labels=["Win", "Loss"],
                    values=[m["win_trades"], m["loss_trades"]],
                    hole=0.6,
    ...
    with c1:
        ...
        if not mr.empty:
            ...
            fig = go.Figure(
                data=[go.Bar(x=mr["month"], y=mr["pnl"], marker_color=colors)]
            )
            fig.update_layout(
                paper_bgcolor=BG,
                plot_bgcolor=BG,
