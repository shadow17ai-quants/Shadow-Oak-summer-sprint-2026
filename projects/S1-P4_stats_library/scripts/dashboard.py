...
with tabs[0]:
    ...
    st.caption(f"Last close: Rs {last_price:,.2f} on {last_date} ({day_chg:+.2%} vs prior close)")
    ...
    with c1:
        try:
            ...
            st.markdown(f"<h2 style='color:{color}'>{ann_ret:.2%}</h2>", unsafe_allow_html=True)
            st.metric("Sharpe", f"{float(report['sharpe_ratio']['sharpe_ratio']):.2f}")
            st.metric("Max Drawdown", f"{float(report['max_drawdown']['max_drawdown']):.2%}")
    ...
    with c2:
        try:
            ...
            fig = go.Figure(data=[go.Scatter(
                x=prices.index, y=[float(v) for v in prices.values], mode="lines",
                line=dict(color=PURPLE, width=2), fill="tozeroy",
                fillcolor="rgba(139,92,246,0.15)",
            )])
            fig.update_layout(paper_bgcolor=BG, plot_bgcolor=BG, font_color=TEXT, height=260,
                               margin=dict(t=10, b=10, l=10, r=10),
                               xaxis=dict(gridcolor="#2a2444"), yaxis=dict(gridcolor="#2a2444"))
    ...
    with c3:
        try:
            ...
            for label, val in metrics:
                val_str = f"{val:.2f}" if val == val and abs(val) != float("inf") else "N/A"
                st.markdown(
    ...
    with c4:
        try:
            ...
            fig = go.Figure(data=[go.Scatter(
                x=cum.index, y=[float(v) for v in cum.values], mode="lines",
                line=dict(color=TEAL, width=2), fill="tozeroy",
                fillcolor="rgba(34,211,238,0.12)",
            )])
            fig.update_layout(paper_bgcolor=BG, plot_bgcolor=BG, font_color=TEXT, height=260,
                               margin=dict(t=10, b=10, l=10, r=10),
                               xaxis=dict(gridcolor="#2a2444"), yaxis=dict(gridcolor="#2a2444", tickformat=".0%"))
            st.plotly_chart(fig, use_container_width=True)
        except Exception as e:
    ...
    with c5:
        try:
            ...
            fig = go.Figure(data=[go.Pie(
                labels=["Up Days", "Down Days"], values=[up, down],
                hole=0.6, marker=dict(colors=[TEAL, PINK]), textinfo="percent",
            )])
            fig.update_layout(paper_bgcolor=BG, plot_bgcolor=BG, font_color=TEXT, height=260,
                               margin=dict(t=10, b=10, l=10, r=10))
            st.plotly_chart(fig, use_container_width=True)
    ...
    try:
        ...
        tests = ["jarque_bera_test", "shapiro_wilk_test", "ljung_box_test", "adf_test", "fit_student_t"]
        labels = [t.replace("_test", "").replace("_", " ").title() for t in tests]
        p_values = [float(report[t]["p_value"]) if report[t]["p_value"] is not None else 0.0 for t in tests]
        colors = [GREEN if p >= 0.05 else RED for p in p_values]
        fig = go.Figure(data=[go.Bar(
            x=labels, y=p_values, marker_color=colors,
            text=[f"{p:.3g}" for p in p_values], textposition="outside",
        )])
        fig.add_hline(y=0.05, line_dash="dash", line_color=MUTED, annotation_text="ÃŽÂ± = 0.05")
        fig.update_layout(paper_bgcolor=BG, plot_bgcolor=BG, font_color=TEXT, height=300,
                           margin=dict(t=30, b=10, l=10, r=10),
                           xaxis=dict(gridcolor="#2a2444"), yaxis=dict(gridcolor="#2a2444", title="p-value"))
        st.plotly_chart(fig, use_container_width=True)
    except Exception as e:
...
with tabs[1]:
    ...
    def ratio_card(title, value, good_if, fmt="{:.2f}", explain_good="", explain_bad=""):
        try:
            v = float(value)
            is_good = good_if(v)
    ...
    with rc1:
        ratio_card("Sharpe Ratio", report["sharpe_ratio"]["sharpe_ratio"], lambda v: v > 1,
        ...
        ratio_card("Calmar Ratio", report["calmar_ratio"]["calmar_ratio"], lambda v: v > 0.5,
        ...
        ratio_card("Max Drawdown", report["max_drawdown"]["max_drawdown"], lambda v: v > -0.15, fmt="{:.2%}",
    ...
    with rc2:
        ratio_card("Sortino Ratio", report["sortino_ratio"]["sortino_ratio"], lambda v: v > 1,
        ...
        ratio_card("Omega Ratio", report["omega_ratio"]["omega_ratio"], lambda v: v > 1,
...
with tabs[2]:
    ...
    try:
        ...
        test_card("Normality (Jarque-Bera)", jb["statistic"], jb["p_value"], jb["p_value"] >= 0.05,
    ...
    try:
        ...
        test_card("Normality (Shapiro-Wilk)", sw["statistic"], sw["p_value"], sw["p_value"] >= 0.05,
    ...
    try:
        ...
        meaning = ("No significant autocorrelation - today's return doesn't meaningfully predict tomorrow's from own history alone (closer to a random walk)."
                   if passed else
                   "Significant autocorrelation detected - past returns carry some predictive signal for future returns. Worth exploring as a momentum/mean-reversion feature.")
        test_card("Autocorrelation (Ljung-Box)", lb["statistic"], lb["p_value"], passed, meaning)
    except Exception as e:
        st.error(f"Ljung-Box card failed: {e}")

    try:
        adf = report["adf_test"]
        test_card("Stationarity (ADF)", adf["statistic"], adf["p_value"], adf["p_value"] < 0.05,
    ...
    try:
        ...
        passed = df >= 5
        meaning = (f"Degrees of freedom = {df:.2f}. Fat tails are extreme (df < 5) - large moves happen far more often than a normal distribution assumes. Size positions and stop-losses with this in mind."
                   if not passed else
                   f"Degrees of freedom = {df:.2f}. Tails are closer to normal - extreme moves are relatively less frequent.")
        test_card("Tail Risk (Student-t fit)", st_t["statistic"], st_t["p_value"], passed, meaning)
    except Exception as e:
