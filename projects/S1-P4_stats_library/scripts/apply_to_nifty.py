...
NIFTY_CSV = Path(__file__).parent.parent.parent / "S1-P2_nifftyvault" / "data" / "nifty_50_data.csv"
...
def main():
    ...
    print(f"Loaded {len(df)} rows, {len(log_returns)} log returns from {NIFTY_CSV.name}")
    print("=" * 70)

    report = full_report(log_returns)

    print("\n--- RATIOS ---")
    for name in ["sharpe_ratio", "sortino_ratio", "calmar_ratio", "omega_ratio", "max_drawdown"]:
        print(f"\n{name}:")
        for k, v in report[name].items():
            print(f"  {k}: {v}")

    print("\n--- HYPOTHESIS TESTS ---")
    for name in ["jarque_bera_test", "shapiro_wilk_test", "ljung_box_test", "adf_test", "fit_student_t"]:
        print(f"\n{name}:")
        for k, v in report[name].items():
            print(f"  {k}: {v}")
