...
def sharpe_ratio(returns, risk_free_rate=0.0, periods_per_year=252):
    ...
    if ann_vol < 1e-12:
        sharpe = (
            np.inf
            if ann_return > risk_free_rate
            else (0.0 if ann_return == risk_free_rate else -np.inf)
        )
    else:
...
def omega_ratio(returns, threshold=0.0):
    ...
    omega = (
        ...
        else (np.nan if losses == 0 else gains / losses)
    )
    return {
        "omega_ratio": omega,
        "gains_sum": float(gains),
        "losses_sum": float(losses),
    }
