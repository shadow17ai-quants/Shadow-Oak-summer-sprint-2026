...
def jarque_bera_test(returns):
    r = _clean(returns)
    if len(r) < 2:
        return {"statistic": np.nan, "p_value": np.nan, "interpretation": "insufficient data"}
    stat, p_value = stats.jarque_bera(r)
    interp = "NOT normally distributed (reject H0)" if p_value < 0.05 else "cannot reject normality (fail to reject H0)"
    return {"statistic": float(stat), "p_value": float(p_value), "interpretation": interp}
...
def ljung_box_test(returns, lags=10):
    ...
    interp = "significant autocorrelation (reject H0)" if p_value < 0.05 else "no significant autocorrelation (fail to reject H0)"
...
def adf_test(returns):
    ...
    interp = "stationary (reject H0 of unit root)" if p_value < 0.05 else "NOT stationary (fail to reject H0 of unit root)"
    return {"statistic": float(stat), "p_value": float(p_value), "interpretation": interp}
...
def fit_student_t(returns):
    r = _clean(returns)
    if len(r) < 5:
        return {"statistic": np.nan, "p_value": np.nan, "degrees_of_freedom": np.nan, "interpretation": "insufficient data"}
    ...
    return {"statistic": float(ks_stat), "p_value": float(ks_p), "degrees_of_freedom": float(df),
            "loc": float(loc), "scale": float(scale), "interpretation": interp}
